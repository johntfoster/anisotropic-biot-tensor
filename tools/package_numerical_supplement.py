...
VERSION = 'anisotropic-biot-2026-09-22-v3'
...
# Refined (40 x 4) contour runs ship both the scalar history and the Exodus
...
def main():
    sources = ['conformal_model.py', 'conformal_experiments.py',
               ...
               'plot_fabric_results.py', 'plot_fabric_contours.py', 'figure_style.py',
               'fabric_insight.py', 'plot_fabric_comparison.py',
    ...
    files += [ROOT / 'fe-evidence/runs' / case / 'solution.e'
              for case in FABRIC_CONTOUR_CASES]
    files += [ROOT / 'tools/rerun_fabric_contours.py', ROOT / 'tools/run_fabric_comparison.py', ROOT / 'tools/verify_scalar_probe.py']
    files += list((ROOT / 'fe-evidence/insight').glob('*/solution.csv'))
    files += list((ROOT / 'fe-evidence/insight').glob('*/provenance.json'))
    files += list((ROOT / 'fe-evidence/insight-probe').glob('*/solution.csv'))
    files += list((ROOT / 'fe-evidence/insight-probe').glob('*/provenance.json'))
    files += list((ROOT / 'moose_app/src').rglob('*.C'))
    files += list((ROOT / 'moose_app/include').rglob('*.h'))
    files += [ROOT / 'moose_app/Makefile', ROOT / 'moose_app/inputs/fabric_comparison.i']
    files += [ROOT / 'LICENSE', ROOT / 'LICENSES.md']
    files += list((ROOT / 'licenses').glob('*'))
    for directory in ('conformal', 'weighted-stress', 'fabric', 'insight'):
        files += [p for p in (ROOT / 'build' / directory).glob('*')
                  if p.suffix in ('.csv', '.dat', '.json', '.pdf', '.pgf', '.tex', '.txt')]
    ...
    for required in ('verification.json', 'comparison_verification.json', 'comparison_summary.tex'):
        if not (ROOT / 'build/insight' / required).is_file():
            raise SystemExit('Run the insight experiments before packaging: ' + required)
...
The three new studies are regenerated by `fabric_insight.py` and
`plot_fabric_comparison.py`. The finite
material benchmark is restricted to commuting mineral stress and distention.
The scalar/tensor consolidation histories are recorded under `fe-evidence/insight`;
plotting verifies their source and history hashes. To recompute those histories,
use the shipped MOOSE application sources with framework commit
`abafb58b67a6037c6723ffeb19647c84484466da` and Conda packages
`moose-dev=2026.02.20`, `moose-libmesh=2026.02.18_f8a1758`,
`moose-tools=2026.02.16`, and `pandas=3.0.1` from the INL and conda-forge
channels. Build with
`make -C moose_app MOOSE_DIR=/path/to/moose`, and run
`python3 tools/run_fabric_comparison.py` from the archive root in that environment.
The MOOSE framework and compiled dependencies are not bundled. The numerical
Python material studies and replotting of recorded histories require only the
listed Python packages.
...
The refined (40 x 4) contour runs also ship their Exodus field files under
