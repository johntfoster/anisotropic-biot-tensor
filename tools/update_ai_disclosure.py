...
def first_manuscript_commit(
    root: Path, paths: list[str], pending_date: str | None = None
) -> dict[str, str]:
    result = subprocess.run(
        ["git", "log", "--reverse", "--format=%H%x09%aI", "--", *paths],
        cwd=root,
        check=False,
        text=True,
        capture_output=True,
    ...
    if not output:
        if pending_date:
            return {"commit": "pending-initial-commit", "date": pending_date}
        detail = result.stderr.strip()
        raise RuntimeError(detail or "no manuscript-touching commit found")
...
def render_markdown(registry: dict[str, Any], first: dict[str, str], end: dict[str, str]) -> str:
    if registry.get("journal_statement"):
        statement = registry["journal_statement"]
        return ("<!-- Generated by tools/update_ai_disclosure.py; edit provenance/ai-use.yml instead. -->\n"
                "# AI-use provenance\n\n" + statement + "\n\n"
                "## Record boundaries\n\n"
                "This working-draft statement includes the current uncommitted revision. "
                "Historical commit/session records remain in this directory; AI review is not journal acceptance.\n")
...
def render_latex(registry: dict[str, Any], first: dict[str, str], end: dict[str, str]) -> str:
    if registry.get("journal_statement"):
        return ("% Generated by tools/update_ai_disclosure.py; edit provenance/ai-use.yml instead.\n"
                ...
                + latex_escape(registry["journal_statement"]) + "\n")
    start = human_date(first["date"])
    finish = human_date(end["date"])
...
def main() -> int:
    ...
    first = first_manuscript_commit(root, registry["manuscript_paths"], args.pending_date)
