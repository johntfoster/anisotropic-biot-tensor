...
\begin{equation}
 ...
 W_d=\frac12\mathbf\varepsilon:\mathbb{C}^d:\mathbf\varepsilon.
 \label{eq:prescribed-logarithmic-energies}
\end{equation}
The first is per reference mineral volume and the second per reference
mixture volume. At zero pressure the coupled energy must recover \(W_d\).
...
Pressure equilibrium is enforced by differentiating this energy at fixed
\(\mathbf F\).
Both \(J\) and skeleton shape then remain fixed while distention changes
with mineral volume. Multiplying the derivative by \(\bar J\) gives
\begin{align}
 -\phi_{s0}p\bar J
...
The spherical drained path fixed \(W_A\). Reproducing the entire drained
Hooke law places a further restriction on \(\mathbb{C}^d\).
...
\(W_d\). Positive mineral stiffness
