...
The finite-deformation relations specialize to the undeformed reference
state and to two limiting experiments. We give the reference Biot tensor
and solid storage first, then specialize to an isotropic mineral, and
finally verify finite homogeneous compression along an unjacketed path.
...
\(\mathbf B_0=(1-\mathcal{K}/\mathcal{K}_s)\mathbf I\).
...
\begin{align}
 ...
 &=\frac{\phi_{s0}}{\mathcal{K}_s}\left(1-\frac{\mathcal{K}}{\phi_{s0}\mathcal{K}_s}\right),
...
The stress relation and the solid storage relation come from the same energy
and the same mineral-volume response.

\subsection{Isotropic mineral}
For an isotropic mineral,
\(\mathbb{C}_s:\mathbf I=3\mathcal{K}_s\mathbf I\). The mineral volume--shape
term vanishes and \eqref{eq:anisotropic-mineral-eos} becomes
\begin{equation}
 \mathcal{K}_s\ln\bar J+\left(1-\frac{\mathcal{K}}{\phi_{s0}\mathcal{K}_s}\right)p\bar J
                   -\frac{\mathcal{K}}{\phi_{s0}}\ln J=0.
 \label{eq:reconstructed-isotropic-source-eos}
\end{equation}
The Biot tensor is spherical,
\begin{equation}
 \mathbf B=\left[1-\frac{\mathcal{K}\bar J}
 {J\left[\mathcal{K}_s+\left(1-\mathcal{K}/(\phi_{s0}\mathcal{K}_s)\right)p\bar J\right]}\right]
 \mathbf I.
...
the volumetric law of Gajo's model. Here the full Hooke law and volume-only spherical distention
...
An unjacketed specimen has confining pressure equal to pore pressure. In
that experiment the solid and the mixture reach the same homogeneous
stretch, and their representations may differ only by the internal rotation.
We therefore choose a state satisfying
\begin{equation}
