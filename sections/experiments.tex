...
For current unit normal \(\mathbf n\) and tangent \(\mathbf t\), define
\begin{equation}
 \mathbf n=(\cos\theta,\sin\theta,0)^T,\qquad
 \mathbf t=(-\sin\theta,\cos\theta,0)^T.
...
\(\mathbf t\cdot\mathbf B\mathbf n\), respectively.
...
The numerical supplement is embedded in the PDF as
\texttt{conformal-\allowbreak2026-\allowbreak09-\allowbreak20-\allowbreak v1.zip}.
...
constitutive verification only. The coupled finite-element implementation of
section~\ref{sec:finite-elements} is not part of that archive: its
application sources, deck inputs, per-run analyses, figure data, and the
machine-readable verification-evidence file travel with the manuscript sources
in the repository
