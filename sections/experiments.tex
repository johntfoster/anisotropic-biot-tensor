...
coefficient. Figure~\ref{fig:conformal-pressure} shows the coefficients,
...
Figure~\ref{fig:conformal-shear} shows that the anisotropic mineral changes
...
\begin{figure}[htbp]
 ...
 \label{fig:conformal-shear}
\end{figure}

The tensor also gives the pressure sensitivity of traction on a plane.
For current unit normal \(\mathbf n\) and unit tangent \(\mathbf e_t\), define
\begin{equation}
...
Figure~\ref{fig:conformal-directional} compares the undeformed skeleton
...
\begin{figure}[htbp]
 ...
 \label{fig:conformal-directional}
\end{figure}

\subsection{Internal and physical rotations}
There are two different rotation experiments. First, hold the physical
...
Figure~\ref{fig:conformal-rotation} shows constant components in the first
...
is coupled to the mineral-volume equation. Figure~\ref{fig:conformal-layer}
...
\begin{figure}[htbp]
 ...
 \label{fig:conformal-layer}
\end{figure}

\subsection{Independent verification and reproducibility}
The stress is first evaluated by differentiating the mineral energy in
...
The conformal verification suite contains 186 named checks. These include the
...
Additional checks recover the isotropic formula, the reference Biot
relation, and the rank-one compliance restriction. The spherical-gauge
suite separately checks 273 finite states across 13 mineral stiffnesses;
...
The implementation is in \nolinkurl{examples/conformal_model.py}.
The scripts \nolinkurl{examples/conformal_experiments.py} and
\nolinkurl{examples/verify_conformal.py} generate the examples and run
the checks, respectively. The repository records the parameter matrices, loading paths,
...
The numerical supplement is embedded in the PDF as the attachment
\texttt{anisotropic-\allowbreak biot-\allowbreak2026-\allowbreak09-\allowbreak20-\allowbreak v2.zip}.
It contains the numerical sources,
parameter matrices, CSV figure data, and verification reports. The archive's
\nolinkurl{manifest.json} identifies its version and SHA-256 hashes of all
payload files; its README gives the reproduction commands. Download the PDF
and use an attachment-capable viewer to save the archive, or extract it with
\texttt{pdfdetach -saveall article.pdf}. This supplement is self-contained
and does not require the companion repository, and it covers the
constitutive and material-point verification. The archive does ship the
pore-fabric constitutive law, the material that registers it, and their
coupled decks under \nolinkurl{moose_app/}: the \nolinkurl{FabricLaw.h}
distention law, the \nolinkurl{FabricMaterial.h} and
\nolinkurl{FabricMaterial.C} material, and the \nolinkurl{fabric_probe.i},
\nolinkurl{conformal_probe.i}, and \nolinkurl{fabric_mandel.i} decks, together
with the recorded run histories those scripts read. Those decks are inputs,
not a runnable build: reproducing them requires the repository application
build, whose remaining sources the archive does not ship. The rest of the coupled
finite-element implementation of section~\ref{sec:finite-elements} is not
part of that archive: its remaining application sources, deck inputs,
per-run analyses, figure data, and the machine-readable
verification-evidence file travel with the manuscript sources
in the repository
...
under \nolinkurl{fe-evidence/runs/} (each case directory holds its input deck,
run provenance, per-run analysis, and scalar history), the manufactured-solution
convergence analysis under \nolinkurl{fe-evidence/mms-convergence.json}, figure
data under \nolinkurl{figures/}, and the verification-evidence file at
\nolinkurl{site/evidence.json}, which records the inspected source revision and
pins the SHA-256 hashes of the implementation and evidence files it reports.
The public source location and inspected commit of the unpublished companion
manuscript are given in reference~\citep{foster2026poroplastic}.
