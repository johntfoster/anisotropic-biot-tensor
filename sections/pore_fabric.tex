...
The conformal specialization of
...
section replaces the scalar dilation with a symmetric positive definite
distention and derives the resulting energy, its work conjugate, the
equilibrium that determines the fabric, and the Biot tensor. We state at
each stage which assumptions are new modeling choices rather than
consequences of the conformal model.
...
\eqref{eq:conformal-mineral-metric}. In that limit the identity stretch
...
datum. Throughout the remainder we express the fabric and the mineral
stiffness in a common material frame, which amounts to fixing
\(\mathbf R_A=\mathbf I\) in
\eqref{eq:fabric-multiplicative}--\eqref{eq:fabric-mineral-metric}. This
is a modeling choice; in the conformal specialization it is immaterial
...
distention, and the orientation of the pore fabric relative to the
mineral enters as prescribed material data rather than as a quantity the
deformation determines. The construction therefore carries no rotational
...
The total energy per reference mixture volume is
\begin{equation}
...
tensor \(\mathbf G\), replacing the volume-only energy \(W_A(a)\). Its
...
\end{align}
In the conformal limit \(\mathbf G^{1/2}=a^{1/3}\mathbf I\) the first term
becomes \(\tfrac13\tr\mathbf\tau'\,\delta\ln a\) and the second
\(\phi_{s0}\widehat{\mathbf\tau}_s:(\delta\bar{\mathbf F}\bar{\mathbf F}^{-1})\),
recovering \eqref{eq:distention-mineral-energy-work} exactly. The first
term of \eqref{eq:fabric-phase-work} is the tensorial distention work; it
reduces to the scalar pressure-like work of the conformal model and adds
the shape-changing work of the fabric.

\subsection{Equilibrium and material symmetry}
\label{sec:fabric-equilibrium}

At fixed deformation and pressure the fabric is determined by the
equilibrium that minimizes the potential over \(\mathbf G\):
\begin{equation}
 \pder{}{\mathbf G}
...
complement \(\mathbb{D}\) vanishes, the potential does not depend on the
complementary modes, and those modes are frozen to the mineral rather than
equilibrated. Equation \eqref{eq:fabric-equilibrium} is therefore read on
that subspace, where the quadratic potential is strictly convex and its
stationary point exists and is unique. Differentiating along the conformal
...
distention stress \eqref{eq:fabric-distention-stress} is symmetric. For
the fabric, not an arbitrary frame, to govern the distention energy,
\(W_{\mathrm{dis}}\) must be an isotropic scalar function of the fabric tensor, that
is, depend only on the three invariants
\(\tr\mathbf G\), \(\tr\mathbf G^2\), \(\tr\mathbf G^3\). If the mineral
carries its own material symmetry group with structural tensors,
\(W_{\mathrm{dis}}\) is instead an isotropic function of \(\mathbf G\) and those
structural tensors. This is the fabric-tensor requirement of
\citet{cowin1985fabric}, applied to the distention energy rather than to
the stiffness; it is the generalization of the conformal assumption that
\(W_A\) depends only on \(a=(\det\mathbf G)^{1/2}\).
...
\(\mathbf G(\mathbf F,p)\). The result is a symmetric tensor whose
directional pressure response carries both mineral anisotropy and fabric
anisotropy. In the conformal model the shape terms were absent because
\(\mathbf G\) had only the volume degree of freedom; here they are present
and are determined by the distention stiffness.

The rank-one structure of the conformal compliance restriction relaxes. In
...
\eqref{eq:fabric-equivalent-energy} is therefore over
...
mineral, which carries them at its own compliance. On that subspace the
quadratic potential is strictly convex, so the stationary point of
\eqref{eq:fabric-equilibrium} is unique and minimizing at fixed
\(\mathbf\varepsilon\) gives the drained compliance
\begin{equation}
 (\mathbb{C}^d)^{-1}
...
of the fabric. In the conformal case \(W_{\mathrm{dis}}\) depends only on
...
invariant under rotations about \(\mathbf m\); equivalently, it annihilates
the four complementary modes of this basis, the coupled in-plane pair
...
\(\operatorname{sym}(\mathbf m\otimes\mathbf p_1)\) and
\(\operatorname{sym}(\mathbf m\otimes\mathbf p_2)\). The labels
...
scalar is the logarithm of the unimodular transverse fabric eigenvalue
\(h=\mathrm{e}^{\ln h}\), not a projection of it.
The equilibrium \eqref{eq:fabric-equilibrium} splits into a volume balance
for \(a\), which couples to pressure and reduces to the conformal pressure
...
the conformal model. The extension therefore predicts that a
