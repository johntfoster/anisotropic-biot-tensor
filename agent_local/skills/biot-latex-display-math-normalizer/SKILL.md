---
name: biot-latex-display-math-normalizer
...
```bash
python3 agent_local/skills/biot-latex-display-math-normalizer/scripts/scan_display_math.py --root . --files sections/target.tex
```
...
Read `author_style_profile.md` and the notation rules in `AGENTS.md`. The
canonical root is `main.tex`, with macros in its preamble, and the sole
preview is `build/main.pdf`. Local rules require numbered, descriptively
labeled displays and `align` for multistep derivations. Use existing physical
symbols; fourth-order tensors use `\mathbb{}` and second-order tensors
and vectors use upright bold `\mathbf{}`. Follow the volume-fraction-
weighted spatial stress balance and show the energy-work equivalence before
introducing a constitutive energy. A scalar reduction alone does not verify
a tensor balance.
